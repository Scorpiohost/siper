...
@section('content')
    <x-cards.card title="Edit Data">
        <x-forms.form action="" method="post">
            <x-forms.r_input label="Koderesi" type="text" :value="$data->koderesi" />
            ...
            <div class="form-group">
                ...
                <select name="Tcodeout" id="Tcodeout" class="form-control @error('Tcodeout') is-invalid @enderror">
                    ...
                    @foreach ($barang_dikirim as $result)
                        <option value="{{ $data->tcodeout }}" @if ($data->tcodeout == $result->tcodeout) selected @endif>{{ $result->tcodeout }}</option>
                    @endforeach
                </select>
            </div>
            @error('Tcodeout')
            ...
            <div class="form-group">
                ...
                <select name="Pelanggan" id="Tcodeout" class="form-control @error('Pelanggan') is-invalid @enderror">
                    <option value="">-- Pilih --</option>
                    @foreach ($pelanggan as $result)
                        <option value="{{ $data->id_pelanggan }}" @if ($data->id_pelanggan == $result->id) selected @endif>{{ $result->nama }}</option>
                    @endforeach
                </select>
            </div>
            @error('Tcodeout')
            ...
            <div class="form-group">
                ...
                <select name="Kurir" id="Kurir" class="form-control @error('Kurir') is-invalid @enderror">
                    ...
                    @foreach ($kurir as $result)
                        <option value="{{ $data->id_kurir }}" @if ($data->id_kurir == $result->id) selected @endif>{{ $result->kurir }}</option>
                    @endforeach
                </select>
            </div>
            @error('Tcodeout')
            ...
            <x-forms.v_input label="Dikirim" type="date" :value="$data->tanggal_dikirim" />
            <x-components.primary_button>
                Submit
            </x-components.primary_button>
            <x-components.link link="penjualan/barang-dikirim">
